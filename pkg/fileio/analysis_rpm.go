...
func parseRPMHeaders(data []byte) (map[string]any, error) {
	...
	if len(data) < leadSize+16 { return nil, errors.New("file too small for rpm") }
	...
	readHeader := func(start int) (nIndex, hSize int, next int, err error) {
		if start+16 > len(data) { return 0,0,0, errors.New("truncated header") }
		if data[start] != magic0 || data[start+1] != magic1 || data[start+2] != magic2 { return 0,0,0, errors.New("bad header magic") }
		...
		if storeEnd > len(data) { return 0,0,0, errors.New("truncated store") }
		next = storeEnd
		// align to 8
		if rem := next % 8; rem != 0 { next += (8 - rem) }
		return
	}
	// signature header
	nIdx, hSz, next, err := readHeader(off)
	if err != nil { return nil, fmt.Errorf("signature header: %w", err) }
	off = next
	_ = nIdx; _ = hSz // unused currently
	// main header
	nIdx, hSz, next, err = readHeader(off)
	if err != nil { return nil, fmt.Errorf("main header: %w", err) }

	entriesStart := off + 16
	storeStart := entriesStart + nIdx*16
	storeEnd := storeStart + hSz
	if storeEnd > len(data) { return nil, errors.New("truncated main store") }
	store := data[storeStart:storeEnd]

	getString := func(offset int) string {
		if offset < 0 || offset >= len(store) { return "" }
		end := offset
		for end < len(store) && store[end] != 0 { end++ }
	...
	for i := 0; i < nIdx; i++ {
		...
		if typeID == RPM_STRING || typeID == RPM_I18NSTRING {
			...
			switch tag {
			case TAG_NAME: meta["name"] = val
			case TAG_VERSION: meta["version"] = val
			case TAG_RELEASE: meta["release"] = val
			case TAG_SUMMARY: meta["summary"] = val
			case TAG_LICENSE: meta["license"] = val
			case TAG_ARCH: meta["arch"] = val
			case TAG_PAYLOADFORMAT: meta["payload_format"] = val
			case TAG_PAYLOADCOMPRESSOR: meta["payload_compressor"] = val
			case TAG_PAYLOADFLAGS: meta["payload_flags"] = val
			}
		}
	}
	return meta, nil
